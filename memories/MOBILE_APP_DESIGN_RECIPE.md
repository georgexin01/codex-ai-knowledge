...
### Step 1.1 — Create folder + write configs (parallel writes)

Create these 7 files at the project root:
...
```json
{
  "compilerOptions": {
    ...
    "useDefineForClassFields": true, "strict": true, "jsx": "preserve",
    "resolveJsonModule": true, "isolatedModules": true, "esModuleInterop": true,
    "lib": ["ES2023", "DOM", "DOM.Iterable"], "skipLibCheck": true,
...
Run in background while you write the rest of the files.
...
export default {
  ...
  theme: {
    extend: {
      colors: {
        ink:    { DEFAULT: "#10171c", soft: "#1a232a" },
        snow:   { DEFAULT: "#f7f7f7", warm: "#fafafa" },
        sky:    { DEFAULT: "#97cff3", soft: "#cfe7f8" },
        mint:   { DEFAULT: "#a7e8d1", soft: "#d1f1e3", deep: "#5fc9a3" },
        gold:   { DEFAULT: "#f2da1b", soft: "#fbeb6b" },
        active: "#22c55e",
      },
      fontFamily: { sans: ['"DM Sans"', "sans-serif"] },
      borderRadius: { "4xl": "2rem", "5xl": "2.5rem" },
      boxShadow: {
        card: "0 18px 40px -16px rgba(16,23,28,0.18)",
        soft: "0 8px 24px -8px rgba(16,23,28,0.08)",
      },
    },
  },
  plugins: [],
...
> **Theme adjustment hint:** swap `mint`/`gold` to alternate accent pair if the brief is non-fintech. Keep `ink` and `snow` regardless — they're the structural anchors. Do not introduce more than one warm + one cool accent unless explicitly needed.
...
<html lang="en" dir="ltr">
  <head>
    ...
    <meta name="viewport"
      content="width=412, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover" />
    ...
    <link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@24,200..400,0..1,0&display=swap" rel="stylesheet" />
    <style>
      * { -webkit-tap-highlight-color: transparent; scrollbar-width: none; }
      *::-webkit-scrollbar { display: none; }
      body { font-family: "DM Sans", sans-serif; background-color: #ececec; overscroll-behavior-y: contain; }
      .material-symbols-outlined { font-variation-settings: "FILL" 0, "wght" 300, "GRAD" 0, "opsz" 24; }
      #app {
        max-width: 412px; margin: 0 auto; min-height: 100vh; background-color: #f7f7f7;
...
> **CRITICAL** — `width=412` is mandatory per [CLAUDE.md](C:/Users/user/.claude/CLAUDE.md). Do NOT use `width=device-width`.

### Step 2.3 — `src/style.css`
...
### Step 3.1 — Toast system (build FIRST, before any view)

**`src/composables/useToast.ts`**
```ts
import { reactive } from "vue";
export type ToastType = "success" | "error" | "info" | "warning";
export interface Toast { id: number; type: ToastType; title: string; message?: string; duration?: number; }
...
export function useToast() {
  const push = (t: Omit<Toast, "id">) => {
    ...
    if (toast.duration && toast.duration > 0) window.setTimeout(() => dismiss(id), toast.duration);
    return id;
  };
  ...
  return {
    toasts: state.toasts, push, dismiss,
...
**`src/components/ToastContainer.vue`** — full file in [reference template](c:/Users/user/Desktop/insurance-CRM/template/src/components/ToastContainer.vue). Key bits:

- `<Teleport to="body">` so it floats above everything
- `fixed top-0 left-1/2 -translate-x-1/2 max-w-[412px] z-[100]`
- `<TransitionGroup>` with `enter-from-class="-translate-y-4 opacity-0 scale-95"`
- 4 variants: `success → bg-mint`, `error → bg-red-500`, `warning → bg-gold`, `info → bg-ink`
- 4 icons: `check_circle / error / warning / info`
- Tap toast to dismiss; auto-dismiss after 3 s

### Step 3.2 — `src/App.vue`

Manual `currentPage` state machine. The shape:
...
const currentPage = ref<string>("onboarding");
const previousPage = ref<string>("home");
const handleNavigate = (page: string) => {
  if (currentPage.value !== page) previousPage.value = currentPage.value;
  currentPage.value = page;
};
const handleBack = () => {
  const main = ["home", "cards", "wallet", "analytics", "profile"];
  currentPage.value = main.includes(previousPage.value) ? previousPage.value : "home";
};
const showBottomNav = () => ["home","cards","wallet","analytics","profile"].includes(currentPage.value);
</script>

<template>
  <div class="flex flex-col min-h-screen">
    <main class="flex-1 overflow-y-auto no-scrollbar relative">
      <OnboardingView v-if="currentPage === 'onboarding'" @navigate="handleNavigate" />
      <!-- ... v-else-if for every page key ... -->
    </main>
    <BottomNav v-if="showBottomNav()" :active-tab="currentPage" @tab-change="handleNavigate" />
    <ToastContainer />
  </div>
</template>
```

### Step 3.3 — `src/components/BottomNav.vue`

4-tab layout, **no FAB** (the `+` lives in Home's Activities card):

- Container: `fixed bottom-0 left-0 right-0 z-40 max-w-[412px] mx-auto bg-snow border-t border-slate-100`
- Inner: `flex items-center justify-around h-16 px-2`
- Each tab: `flex-1 h-full flex items-center justify-center` + active `text-ink` else `text-slate-300`
- Icon: `material-symbols-outlined text-[28px]`, active gets `.fill-1`
- Tab keys: `home`, `cards`, `analytics`, `profile`
- Tab icons: `home`, `credit_card`, `monitoring`, `person`

---

## PHASE 4 — Auth flow (6 views)

Build in this order: Onboarding → Login → Signup → OTP → ForgotPassword → Terms.

### View shape rules

- Each emits `@navigate(page)` to switch route, `@back()` to return to previous
- Each starts with a sub-page header (back button + centered title + 11 px right-side action) **except** Onboarding (full-bleed splash)
- Form views import `useToast` and call `toast.success / error / warning / info` on submit
- Auth views are **never** hidden behind BottomNav (handled in App.vue's `showBottomNav` whitelist)

### Required toast triggers

| View | Action | Toast type | Title | Message |
|---|---|---|---|---|
| Login | submit OK | success | "Welcome back!" | "You're signed in as <name>" |
| Login | empty fields | error | "Missing details" | "Enter both email and password" |
| Signup | submit OK | success | "Account created" | "Now verify your phone with the OTP" |
| Signup | terms not accepted | warning | "Accept terms" | "Tap the box to accept Terms of Service" |
| OTP | verify OK | success | "Verified" | "Welcome to <App>!" |
| OTP | resend | info | "Code sent" | "Check your messages" |
| Forgot | submit | success | "Reset link sent" | "Check ${email} for instructions" |
| Terms | accept | success | "Terms accepted" | "Continue creating your account" |

### Special primitives

- **OTP boxes**: 6× `aspect-square rounded-2xl bg-white border-2` inputs. Use `ref="inputs"` array + `onInput` handler to auto-advance focus. `Backspace` jumps back. Countdown via `setInterval`, stored in `countdown.value`, decremented from 45.
- **Password strength meter**: `computed` returning 0-4 based on length ≥ 8, has-uppercase, has-digit, has-symbol. Render as 4× `h-1 flex-1 rounded-full` bars colored by score.
- **Sticky bottom action bar** (Terms accept): `sticky bottom-0 left-0 right-0 bg-snow/90 backdrop-blur-md border-t border-slate-100 px-6 py-4 flex gap-3`. Two pills: outlined Decline + filled ink Accept.

---

## PHASE 5 — Main tabs (5 views)

Build order: Home → Cards → Wallet → Analytics → Profile.

### Page anatomy (every main tab follows this)

```
1. Header row (px-6 pt-12) — avatar/menu left, title or pill center, action icons right
2. Hero section (px-6 mt-6) — primary value, big bold tnum number
3. Quick actions (4-up icon-circles or 3-up cards) OR data hero block
4. Secondary content sections — each starts with `flex justify-between` (h2 bold + tappable link/chip)
5. List rows (rounded-2xl bg-white border border-slate-100 p-3) — icon-circle + title/sub + amount/chevron
6. Optional dark-card "AI insight" or promo card
```

### Section variations per tab

| Tab | Hero | Content sections (order) |
|---|---|---|
| Home | Visa wallet card with mint SVG wave + 4 quick actions | Activities · Spending stacked-bar · Subscriptions · Savings goals (2-up gauges) · Cashback promo (gold) |
| Cards | 3-card stack `-mt-8` overlap (ink → mint → gold) | Add-card composite · Spending limit gauge · Card activity · Rewards (mint) · Benefits list |
| Wallet | $12,000 centered with `.00` faded + 3-action grid (gold Transfer + 2 outlined) | Transactions · Nov summary (2-up) · Upcoming bills with Pay button (toast) |
| Analytics | SVG donut + center amount + legend | Brand transactions · Monthly trend bars · Top categories with progress · Insight card (dark) |
| Profile | Dark hero card + avatar + 3-stat strip | 10-item menu list · Settings CTA |
...
const stroked = segments.map((s) => {
  ...
  const dashoffset = c * 0.25 - acc;  // start at top
  acc += len;
  return { ...s, dasharray, dashoffset };
});
```

Render: `<svg viewBox="0 0 200 200" class="size-full -rotate-90">` + one `<circle stroke="#f1f5f9">` track + one `<circle :stroke-dasharray="s.dasharray" :stroke-dashoffset="s.dashoffset">` per segment.

### Circular gauge (savings goals)

```html
<div class="relative size-16 mx-auto">
  <svg viewBox="0 0 60 60" class="size-full -rotate-90">
    <circle cx="30" cy="30" r="26" stroke="#f1f5f9" stroke-width="6" fill="none" />
    <circle cx="30" cy="30" r="26" stroke-width="6" fill="none" stroke-linecap="round"
            class="stroke-mint"
            :stroke-dasharray="`${(saved / target) * 163.36} 163.36`" />
  </svg>
  <div class="absolute inset-0 grid place-items-center text-[11px] font-bold tnum">
    {{ Math.round((saved / target) * 100) }}%
  </div>
</div>
```

`163.36` = `2 * π * 26`. Pre-compute or inline.

---

## PHASE 6 — Account management (4 views)

ProfileEdit · Settings · Invite · Report.

### ProfileEdit

- Avatar with bottom-right camera button (`size-9 rounded-full bg-ink text-white border-4 border-snow`)
- 6 form fields, each: leading `material-symbols` icon + `bg-transparent outline-none` input
- Email field has trailing `Verified` chip (`bg-active/15 text-active`)
- KYC mint card at bottom of form (`size-10 ink-circle + 2-line text`)
- Sticky bottom Save button (`fixed bottom-0 ... bg-snow/90 backdrop-blur-md`)
- Save → toast.success("Profile updated", "Your changes are live")

### Settings

5 sections, each: uppercase eyebrow label + `rounded-3xl bg-white border border-slate-100 divide-y divide-slate-50` list

| Section | Items |
|---|---|
| Account | Personal info → profile-edit · Biometric login (toggle) · Change password · 2FA (Enabled chip) |
| Notifications | Push (toggle) · Email digest (toggle) · Sounds (toggle) |
| Appearance | 3-up theme picker (Light / Dark / Auto) — each card is `aspect-[5/3] rounded-lg` mini-preview |
| Preferences | Language (chevron) · Currency (chevron) |
| About | Terms & Privacy → terms · Help Center · App version |

Plus a **Danger zone** (`border-red-100 divide-red-50`): Logout (toast.info → login), Delete account.
...
- Dark hero (`rounded-3xl bg-ink text-white p-6 + 2 blob accents`) — gold "Limited time" chip + "Invite a friend, earn $10 each" headline
- Code card: `rounded-2xl bg-mint/30 border-2 border-dashed border-mint-deep` + `text-[20px] font-black tracking-[0.4em] tnum`
- Copy button → `navigator.clipboard.writeText` + toast.success("Code copied")
- 4-up share grid: WhatsApp (#25D366), Email (sky), SMS (mint), More (ink)
- 3-step how-it-works numbered list
- 3-up stats: Invited / Joined / Earned (last is `bg-mint`)
- Recent referrals list

### Report

- Dark `rounded-b-[40px]` hero header — back button + title + mint download button
- Hero amount: $10,984.25 with `.25` faded
- 2-up income/spent inside `bg-white/10 backdrop-blur` chips
- 6-month trend bars (current month `bg-mint`, others `bg-slate-100`, height = pct%)
- Category breakdown with per-row progress bars
- Goals list with status pills (`bg-active/15 text-active` On track, `bg-red-100 text-red-500` At risk/Behind)
- Mint AI insight card with size-10 ink icon-circle
- Full-width dark Export PDF CTA (toast on click)

---

## PHASE 7 — Sub-pages (5 views)

History · Calendar · Products · TransactionDetail · Notifications.

### History

- Search input: `rounded-full bg-white border border-slate-100 px-4 py-2.5` + leading search icon
- 4 horizontal-scroll filter chips: `shrink-0 px-4 py-1.5 rounded-full text-[12px] font-semibold` — active `bg-ink text-white`, inactive `bg-white border border-slate-100`
- 3-up summary: Spent (red-500) / Earned (active) / Net (mint)
- Date groups: uppercase eyebrow + `text-[11px] text-slate-400` count badge + `space-y-2` items
- Each row tappable → navigate("transaction-detail")

### Calendar

- Month nav: 2× `size-9` chevron buttons + center `text-[18px] font-bold` month-year
- Day-of-week headers: `text-[11px] font-semibold text-slate-400`
- Grid: `grid-cols-7 gap-1`, each cell `aspect-square` with `rounded-xl`
- States:
  - `today + !selected` → `bg-mint/40 text-ink font-bold`
  - `selected` → `bg-ink text-white font-bold`
  - regular → `text-slate-700 active:bg-slate-50`
- Event dots: 3 colors (gold Bills / mint Card / sky Savings) at `bottom-1 size-1 rounded-full`
- Below grid: selected-day events list — each row gets a colored `w-1 h-12 rounded-full` accent bar

### Products

- Featured dark hero with gold "Featured" chip + 2 blob accents + price + mint CTA
- 6-product 2-col grid — each `rounded-3xl p-4 active:scale-[0.98]` with bg variants (mint / ink / gold / sky / white)
- Each card: `size-11 rounded-2xl bg-white/15 backdrop-blur-sm` icon + title + sub + detail + small underlined CTA with arrow
- "Why us?" benefits list (3 items, each with mint/sky/gold icon-circle + green check_circle right)

### TransactionDetail

- Hero (centered): `size-20 rounded-full bg-mint/40` + 34 px icon + merchant name + 40 px bold tnum amount + status pill
- Details list: `rounded-3xl bg-white border border-slate-100 divide-y divide-slate-50` — 6 rows of `flex justify-between px-5 py-3.5` (label gray + value bold)
- Receipt block: title + 3 line items + dashed-border total
- 3-up action grid: Receipt / Repeat / Report (last is `text-red-500`)

### Notifications

- 3-tab segmented control (All / Unread / Promo) with red unread-count badge
- 2 groups (Today + Earlier) with uppercase eyebrows
- Each row: `size-11 icon-circle` (varied bg by category) + title + msg + time + optional inline CTA pill + unread red dot

---

## PHASE 8 — Verification checklist

After all 20 views written, run:
